                            ...
                            long M, long N, long K, long LN)
{
    ...
    long LK = K;
    void *ptr, *ptr1;
    posix_memalign(&ptr, 64, GEMM_K * 12 *sizeof( float ));
    ...
    for (i = 0; i < LN; i = i + N)
    {
        for (k = 0; k < LK; k = k + kc)      // B block: GK x N
        { 
            kc = GEMM_K;
            if (LK - k < GEMM_K) kc = LK - k;
            ...
            {
                mc = GEMM_M;
                if (M - j < GEMM_M) mc = M - j;
                  
                float *AA = A + k + j * LK; // A[j][k]
                float *CC = C + i + j * LN; // C[j][i]

                SMM_packf32_A(AA, Ac, mc, kc, LK);
